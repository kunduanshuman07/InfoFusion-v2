'use client'
import { addQuizQuestions, adminAddQuiz } from "@/app/apis/adminAddQuiz";
import { useState } from "react";
const AddQuiz = () => {
  const [quizTitle, setQuizTitle] = useState<any>('');
  const [quizDesc, setQuizdesc] = useState<any>('');
  const [descLink, setDesclink] = useState<any>('');
  ...
  const handleSubmit = async (event: any) => {
    ...
    const { status, data: { quizId } } = await adminAddQuiz({ title: quizTitle, quiz_index: 1, quiz_description: quizDesc, desc_link: "", quizData });
    if (status == 200) {
      quizData.map((question: any) => {
        addQuizQuestions({ quiz_id: quizId, question: question });
      })
    }
  };
    ...
    <>
      <div className='flex flex-col items-center'>
        <div className='flex flex-col w-full max-w-lg mt-2'>
          ...
          />
          <label htmlFor='quizTitle' className='text-center mb-1 mt-2'>Quiz Description</label>
          <input
            type='text'
            id='quizTitle'
            value={quizDesc}
